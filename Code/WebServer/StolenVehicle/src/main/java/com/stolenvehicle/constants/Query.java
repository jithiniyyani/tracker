...
public class Query {
	...
	public static final String GET_FIND_INFO_BY_ID = "select distinct fi.id,fi.locators_name ,fi.find_location_cordinates,fi.locators_contactNumber,fi.locators_email,v.id,ti.id from user u, vehicle v,theft_information ti,find_information fi where ti.vehicle_id = v.id and ti.status = 'LOST' and ti.id = fi.theft_information_id  and fi.id = ? order by fi.status;";
