...
public class TheftInformationController {
	...
	private static final Logger LOGGER = Logger.getLogger(TheftInformationController.class);
	...
	public ResponseEntity<String> reportTheft(@RequestBody String jsonRequest, HttpServletRequest request) {

		ResponseEntity<String> response = null;
		try {

			AppUtil.checkIfUserHasSession(request);
			TheftInformationTo theftInformationTo = JsonUtil.toObject(jsonRequest, Constants.THEFT_INFO,
					TheftInformationTo.class);

			vehicleService.saveVehicle(theftInformationTo.getVehicle());
			theftInformationTo.setVehicle_id(theftInformationTo.getVehicle().getId());
			theftInformationService.saveTheftInformation(theftInformationTo);
			List<AttachmentTo> saveAttachments = attachmentService.saveAttachments(
					AppUtil.getAttachmentListForTheft(request), theftInformationTo.getVehicle_id(), null);
			...
			LOGGER.error("Error while registering theft request : " + jsonRequest, ex);
	...
	public ResponseEntity<String> getTheftInformation(@RequestParam(name = "theftId") String theftId) {
		ResponseEntity<String> response = null;
		try {
			TheftInformationTo theftInformation = theftInformationService.getTheftInformation(theftId);
			response = new ResponseEntity<String>(JsonUtil.toJson(Constants.THEFT_INFO, theftInformation),
					HttpStatus.OK);

		} catch (Exception ex) {

			LOGGER.error("Error while fetching theftinformation with id " + theftId, ex);
			response = ExceptionProcessor.handleException(ex);
		} finally {
	...
	public ResponseEntity<String> rewardUserForFind(@RequestBody String jsonRequest, HttpServletRequest request) {
		ResponseEntity<String> response = null;
		try {

			AppUtil.checkIfUserHasSession(request);
			RewardTo reward = JsonUtil.toObject(jsonRequest, Constants.REWARD, RewardTo.class);
			String findId = reward.getFindInformationId();
			boolean updateTheftInformation = theftInformationService.updateTheftInformation(reward.getTheftId(),
					reward.getFindInformationId(), TheftStatus.REWARDED);
			boolean updateFindInformationStatus = findInformationService.updateFindInformationStatus(findId,
					FindStatusEnum.REWARDED);
			if (updateTheftInformation == true && updateFindInformationStatus == true) {
			...
			LOGGER.error("Error while getting theft info with request id " + request, ex);
			response = new ResponseEntity<String>(
	...
	public ResponseEntity<String> uploadAttachmentsForTheft(HttpServletRequest request) {
		...
		try {
			...
			for (String fileName : keySet) {
				...
				if (!file.isEmpty()) {
					...
					attachmentList.add(new AttachmentTo(file.getOriginalFilename(), folder,
							web_server_url + user.getId() + "/" + tnxId + "/" + fileName, AttachmentTypeEnum.THEFT));
				}
			}
			...
			auditService.audit(user.getEmailaddress(), AuditEnum.UPLOAD_ATTACHMENT_FOR_THEFT,
	...
	public ResponseEntity<String> uploadAttachmentsForAnonTheft(HttpServletRequest request) {
