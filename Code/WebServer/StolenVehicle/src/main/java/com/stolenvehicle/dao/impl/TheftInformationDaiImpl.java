...
@Service
public class TheftInformationDaiImpl extends AbstractDao implements TheftInformationDao {

	private static final class TheftInformationResultSetExtractor implements ResultSetExtractor<TheftInformation> {

		@Override
		public TheftInformation extractData(final ResultSet resultSet) throws SQLException {
			...
			if (resultSet.next()) {
				...
				theftInformation.setTheft_dateTime(resultSet.getTimestamp("ti.theft_dateTime"));
				theftInformation.setTheft_location_cordinates(resultSet.getString("ti.theft_location_cordinates"));

				theftInformation.setStatus(TheftStatus.valueOf(resultSet.getString("ti.status")));
				theftInformation.setRewards(resultSet.getString("ti.rewards"));
				User user = new User();
				user.setName(resultSet.getString("u.name"));
				user.setId(resultSet.getString("u.id"));
				user.setGender(GenderEnum.valueOf(resultSet.getString("u.gender")));
				theftInformation.setUser(user);
				Vehicle vehicle = new Vehicle();
				vehicle.setType(VehicleEnum.valueOf(resultSet.getString("v.type")));
				...
				vehicle.setRegistrationNo(resultSet.getString("v.registrationNo"));
				vehicle.setExtra_info(resultSet.getString("v.extra_info"));
	...
	private static final class TheftInformationListResultSetExtractor
			implements ResultSetExtractor<List<TheftInformation>> {

		@Override
		public List<TheftInformation> extractData(final ResultSet resultSet) throws SQLException {

			List<TheftInformation> theftInformationList = new ArrayList<>();
			TheftInformation theftInformation = null;
			while (resultSet.next()) {
				...
				vehicle.setRegistrationNo(resultSet.getString("v.registrationNo"));
				theftInformation.setVehicle(vehicle);
				theftInformationList.add(theftInformation);
			}
			return theftInformationList;
		}
	};
	...
	public TheftInformation saveTheftInformation(TheftInformation theftInformation) throws BusinessException {
		...
		this.save(Query.SAVE_THEFT,
				new Object[] { theftInformation.getId(), theftInformation.getTheft_location_cordinates(),
						theftInformation.getVehicle_id(), theftInformation.getCountry_id(),
						theftInformation.getRewards(), theftInformation.getTheft_dateTime(),
						theftInformation.getStatus().toString(), null, Constants.APP_NAME });
		return theftInformation;
	}
	...
	public TheftInformation getTheftInformation(String theftInformationId) throws BusinessException {
		...
		final Object theftInfoOjbect = this.fetch(Query.GET_THEFT_INFO_BY_ID, new Object[] { theftInformationId },
		...
		if (theftInformation == null) {
			...
			final Object attachmentList = this.fetch(Query.GET_VEHICLE_ATTACHMENTS_FOR_THEFT,
					new Object[] { vehicle_id }, new TheftInformationAttachmentsResultSetExtractor());
			theftInformation.getVehicle().setAttachments((List<Attachment>) attachmentList);
		}
	...
	public TheftInformation getTheftInformationByVehicleRegistrationNumber(String registrationNumber)
		...
		final Object theftInfoOjbect = this.fetch(Query.GET_THEFT_INFO_BY_REGISTRATION_NUMBER,
				new Object[] { registrationNumber }, new TheftInformationResultSetExtractor());
		TheftInformation theftInformation = (TheftInformation) theftInfoOjbect;
		if (theftInformation == null) {
	...
	public boolean updateTheftInformationStatus(String theftId, String findId, TheftStatus theftStatus)
		...
		int count = this.save(Query.UPDATE_THEFT_INFO_STATUS, new Object[] { theftStatus.toString(), findId, theftId });
	...
	public List<TheftInformation> getTheftInformationBySearchTo(SearchTo searchTo) throws BusinessException {
		...
		final Object theftInfoListOjbect = this.fetch(Query.GET_THEFT_INFO_BY_SEARCH,
				new Object[] { searchTo.getType().toString(), searchTo.getMake(), searchTo.getCountry_id() },
				new TheftInformationListResultSetExtractor());
		List<TheftInformation> theftInformationList = (List<TheftInformation>) theftInfoListOjbect;
